...
async function main() {
  ...
  while (true) {
    try {
      ...
      const h1Sorted = h1.sort((a, b) => b[0] - a[0]);
      const biasInfo = detectH1Bias(h1Sorted);
      ...
      if (!state.m15BosDone) {
        ...
        const m15Sorted = m15.sort((a, b) => b[0] - a[0]);

        const bosTs = detectM15Bos(m15Sorted, state.h1Bias);
        if (!bosTs) {
          await sleep(1000);
          continue;
        }
        ...
        console.log(
          '🔥 M15 BOS CONFIRMED at',
          new Date(bosTs * 1000).toISOString()
        );
        continue;
      }
      ...
      const m1Sorted = m1.sort((a, b) => b[0] - a[0]);
      console.log(m1);
      console.log('get rates');

      if (!spreadOk(m1Sorted?.[1])) {
        await sleep(1000);
        continue;
      }

      const m5 = await getRates('M5', 3);
      const m5Sorted = m5.sort((a, b) => b[0] - a[0]);
      const order = buildOrderFromM5(m5Sorted, state.h1Bias, tick);
      if (!order) {
        await sleep(1000);
        continue;
      }
