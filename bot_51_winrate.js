...
function spreadOk(lastM1) {
  if (!lastM1) return true;
  return lastM1[6] == null || lastM1[6] <= MAX_SPREAD_POINTS;
}
...
function calcLot(balance, entry, sl, symbolInfo) {
  ...
  let lot = riskMoney / (priceDistance * valuePerPrice);
...
function detectM15Bos(m15, bias) {
  if (m15.length < 3) return false;

  const last = m15[1];
  const prev = m15[2];

  if (bias === "LONG" && last[4] > prev[2]) return true;
  if (bias === "SHORT" && last[4] < prev[3]) return true;

  return false;
}

// ----- M5 ENTRY -----
function buildOrder(m5, bias, tick) {
  ...
  while (true) {
    try {
      ...
      if (!state.m15BosDone) {
        ...
        if (!detectM15Bos(m15, state.h1Bias)) {
          await sleep(1000);
          continue;
        }
      ...
      const order = buildOrder(m5, state.h1Bias, tick);
      if (!order) {
        await sleep(1000);
        continue;
      }
